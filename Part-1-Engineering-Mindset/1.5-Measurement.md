...
status: "Reviewed"
difficulty: Beginner
estimated_time: "120-160 minutes"
prerequisites:
...
Imagine buying a new door. The wall opening is 800 mm wide and the door is 805 mm wide. The difference is only 5 mm - that sounds small, but the door does not fit.

Now picture a wheel axle on your buggy that is just 0.3 mm too large for its bearing (the smooth ring a wheel spins on). Again the error sounds tiny, yet the axle may not enter the bearing at all.

Engineering often happens in these small differences, and a successful build depends on knowing exactly what they are. That means measuring - carefully, and in the right units.

---

# A Story: The Shelf That Was Nearly Right

Suppose you measure a space for a shelf. You glance at a ruler and write down:

```text
About 60 cm
```

You cut the board and it is slightly too long, so you trim it - and now it is slightly too short. You add a bracket to hide the gap. The shelf works, but only after extra effort and a visible bodge.

A careful builder would have asked first:
...
For this project, we will mainly use **millimetres**, abbreviated `mm`.
...
Using one main unit throughout a project reduces mistakes.

> **📚 Learn more**
>
> - BBC Bitesize (KS2/KS3 Maths): search "units of measurement" - how
>   millimetres, centimetres and metres relate, with worked conversions

---

# Why Millimetres?

RC parts are small, so centimetres are often too large for clear design work.
...
These words describe the size of an object in different directions, but there is no universal rule for which direction must be called the length. To avoid confusion, engineers make the meaning clear using:
...
The labels matter, because a list of three numbers without labels is dangerous - nobody can tell which way round the part goes.
...
CAD software (the design program you will use to draw parts - Part 2) often asks for either radius or diameter, so read the field carefully before typing a number.

> **📚 Learn more**
>
> - BBC Bitesize (KS3 Maths): search "parts of a circle" - radius,
>   diameter and circumference, and how they relate
...
> **[Sketch: cross-section of a bearing showing the inner ring, outer ring
> and the balls between them - the inside diameter measured across the
> central hole, the outside diameter across the outer ring, and the width
> across the side]**
...
> **[Sketch: two mounting holes side by side - a long arrow from the centre
> of one to the centre of the other labelled "centre-to-centre", and a
> shorter arrow between their nearest edges labelled "edge gap"]**
...
The two measurements describe different things, so always label which one you mean.

> **☕ Good place to pause.** Stretch or get a drink. So far we have covered
> what to measure and the words for each dimension. The next part of the
> topic moves on to the tools that actually take those measurements.
...
> **[Sketch: a metric ruler close-up - the long numbered centimetre marks
> (0, 1, 2, 3) called out, the ten shorter millimetre marks between each
> pair labelled, and one object measured to 43 mm with an eye looking
> straight down the ending edge]**
...
> **🤔 Think about it.** Lay a ruler flat on a table and line an object up
> with a mark. Read the number with your eye directly above the mark, then
> read it again with your head low and off to one side. The object never
> moved - so why did the number change?

Because your line of sight met the scale at an angle, the mark appeared to shift against whatever was behind it. This false shift is called **parallax error**: the viewing angle makes a mark seem to line up with the wrong value.

To reduce it:

- look straight down at the scale
- keep the ruler close to the object
- avoid measuring from an angle

> **📚 Learn more**
>
> - BBC Bitesize (KS3 Physics): search "taking measurements" - reading
>   scales accurately and keeping your eye above the mark to avoid parallax
...
> **[Sketch: a digital caliper with its parts labelled - the large outside
> jaws below, the small inside jaws above, the depth rod poking from the
> tail, the sliding body, the display, and the zero and unit buttons]**

You do not need to own calipers to start. A good steel ruler measures most of this project to the nearest millimetre, and later in this topic you will build a working cardboard caliper of your own (the Topic Mini Project). If you do buy a pair, an inexpensive plastic or entry-level digital caliper is plenty for a first buggy - a workshop-grade one is not needed yet.
...
Calipers can squeeze them and change the result, so use very light pressure and repeat the reading several times.
...
Both numbers are useful: the nominal size identifies the standard, while the actual size helps you design the fit.

---

# Why Measurements Differ

> **🤔 Think about it.** If you have a ruler or calipers, measure the same
> object five times - taking the tool off and putting it back each time -
> and write down all five numbers. They will not all match. Before you read
> on: the object never changed size, so where do the differences come from?
...
The object did not change between readings. The small differences come from how the measurement was made. Possible reasons include:
...
Small variation is normal. The goal is to understand it and keep it under control.
...
Do not simply repeat until you get the number you expected. That is not measurement - that is **confirmation bias**: seeing what you hoped to see rather than what is really there.
...
Three words describe how far a reading can be trusted. They matter enough that the whole of the next topic is about them, so here is just a first taste.

**Resolution** is the smallest change a tool can show. A digital caliper may display steps of 0.01 mm - but showing two decimal places does not mean every reading is truly correct to 0.01 mm.

**Accuracy** is how close a reading is to the true value. A display can look precise and still be wrong:
...
**Measurement uncertainty** is the honest doubt attached to any reading, because no measurement is ever perfectly exact. For beginner work, a note like this is enough:

```text
Width = 47.2 mm, measured three times
```

> We study resolution, accuracy, precision and uncertainty properly in
> **Topic 1.6 - Accuracy, Precision and Error**. For now, just remember:
> honest notes beat false certainty.
...
Do not measure a hot printed part for a critical fit and assume it will stay the same size. Let it cool first.

> **📚 Learn more**
>
> - BBC Bitesize (KS3 Physics): search "particle model" - why materials
>   expand when warm and shrink when cold as their particles move apart
...
> **☕ Good place to pause.** You now know how to take a measurement well. The
> next few sections are about the other half of the job: writing it down so
> it is still useful tomorrow.
...
Later on, you can even work out an angle from a few length measurements - a maths trick for a future topic.

---

> **☕ Good place to pause.** That is the whole toolkit and every technique.
> Stretch, then gather a ruler and a few small objects - the rest of this
> topic is hands-on, and it ends with a caliper you build yourself.
...
# Topic Mini Project - Cardboard Sliding Caliper 🛠️

You have read how a caliper works. The best way to understand one is to build one. From a cereal box you can make a sliding caliper that really measures - and it drills the exact skills this topic is about: zeroing, keeping the jaws square, and recording honestly.

You will need:

- a cereal box or similar thin card
- a ruler with millimetre marks
- a pencil, scissors, and a paper fastener or a small loop of tape
...
> and ask an adult to help with any tricky cuts.

> **🎬 Watch the build**
>
> - Instructables (instructables.com): search "Make a Cardboard Caliper" -
>   a step-by-step photo build using household card
> - ikatbag (ikatbag.com): search "cardboard callipers" - a clearly
>   photographed version with a printable scale
> - YouTube (with an adult): search "DIY cardboard vernier caliper" - see
>   the sliding jaws move before you cut your own

Build steps:

1. Cut two long strips of card the same width - one is the fixed body, one is the sliding piece.
2. Near one end of the body strip, cut a jaw that sticks out at a right angle. Do the same on the sliding strip, so the two jaws face each other.
3. Copy a millimetre scale from your ruler onto the body strip, starting level with the inside face of the fixed jaw.
4. Lay the sliding strip against the body so it can slide while the jaws stay parallel. Hold the two together with a paper fastener or a small loop of tape - tight enough to stay square, loose enough to slide.

Now use it like the real thing:

1. **Zero it.** Close the two jaws until they just touch. The reading should be 0 mm. If it is not, mark where "zero" really falls and remember to subtract it - that is exactly the zero error a real caliper can have.
2. Choose five household objects - a coin, a rubber, a key, a building brick, the thickness of a book.
3. Measure each object three times, taking the caliper off and back on each time, and record all three readings.

The reflection is where the learning lands. In your notebook:

- Make a proper record for each object: name, feature, the three readings, your chosen value, and "tool: cardboard caliper".
- For one object, also measure it with a real ruler. How close was your cardboard caliper, and where did it struggle?
- Notice what your caliper does NOT have: a fine vernier or a digital scale, so it cannot read to 0.01 mm. Knowing a tool's limit is part of using it honestly - the idea we pick up again in Topic 1.6.

Keep your cardboard caliper on the showcase shelf. It is the first measuring tool you made yourself.
...
- Was the bearing housing meant to be a **press fit** (a hole made a touch smaller than the part so it grips tightly - Topic 1.7)?
...
| Confirmation bias | Seeing what you hoped to see rather than what is really there. |
...
In the next topic, **Topic 1.6 - Accuracy, Precision and Error**, we will explore how much to trust a measurement.
