...
|---|---|
| Length | How long something is, along its longest direction |
| Width | How wide it is, across the length |
| Height | How tall it stands |
| Thickness | How deep a wall, plate or sheet is |
| Diameter | The distance straight across a circle or shaft |
| Depth | How far down a hole, slot or pocket goes |
| Spacing | The distance from one feature to the next, centre to centre |
| Angle | How far one face or edge is turned from another |
...
> **[F1 annotated photograph - Figure 1.5.E: cross-section of a bearing showing the inner ring, outer ring and the balls between them - the inside diameter measured across the central hole, the outside diameter across the outer ring, and the width across the side]**
>
> *Figure 1.5.E - A bearing needs three numbers, not one - inside, outside and width - and ordering the wrong one is a common mistake.*
>
> *Alt text: Cross-section of a bearing with its inside diameter, outside diameter and width each dimensioned.*
...
> **[F3 right-versus-wrong array - Figure 1.5.F: two mounting holes side by side - a long arrow from the centre of one to the centre of the other labelled "centre-to-centre", and a shorter arrow between their nearest edges labelled "edge gap"]**
>
> *Figure 1.5.F - Measure hole positions centre to centre. The gap between their edges changes if either hole does; the centres do not.*
>
> *Alt text: Two mounting holes with a centre-to-centre arrow between their centres and a shorter edge-gap arrow between their nearest edges.*
...
> **[F1 annotated photograph - Figure 1.5.G: a metric ruler close-up - the long numbered centimetre marks (0, 1, 2, 3) called out, the ten shorter millimetre marks between each pair labelled, and one object measured to 43 mm with an eye looking straight down the ending edge]**
>
> *Figure 1.5.G - Ten millimetre marks to every numbered centimetre - and your eye has to look straight down the edge, not across at it.*
>
> *Alt text: A metric ruler close up with centimetre and millimetre marks labelled, and an eye looking squarely down at a 43 mm reading.*
...
> **[F2 cross-section - Figure 1.5.H: a digital caliper with its parts labelled - the large outside jaws below, the small inside jaws above, the depth rod poking from the tail, the sliding body, the display, and the zero and unit buttons]**
>
> *Figure 1.5.H - One tool, four different measurements - and each has its own pair of jaws or rod.*
>
> *Alt text: A digital caliper with outside jaws, inside jaws, depth rod, sliding body, display and buttons all labelled.*
...
> **[F1 annotated photograph - Figure 1.5.A: a digital caliper closed onto a small printed bracket, photographed square on. Callouts label the fixed jaw, the moving jaw, the part sitting squarely between them, and the display. A second inset shows the same measurement being taken badly - the part held at an angle between the jaw tips, with the gap it creates exaggerated]**
>
> *Figure 1.5.A - The jaws must sit flat on the surfaces you are measuring - a part gripped at an angle reads larger than it is.*
>
> *Alt text: A caliper measuring a bracket with fixed jaw, moving jaw and display labelled, beside an inset showing the part held crookedly.*
...
> **[F6 measured drawing - Figure 1.5.B: a top and side view of a battery pack drawn as a solid outline, with a dashed rectangle around it showing the packaging envelope. Each gap between the solid outline and the dashed line is dimensioned and labelled: room for the wires to leave and bend, room for the connector and fingers, room for the strap, and clearance to lift the pack out. The battery's own dimensions are marked separately inside]**
>
> *Figure 1.5.B - The battery is the solid line. The space you must design for is the dashed one - and they are never the same size.*
>
> *Alt text: Top and side views of a battery with a dashed packaging envelope around it, each clearance dimensioned and labelled.*
...
> **[F4 sequence strip - Figure 1.5.C: three panels of the same printed cylinder being measured with calipers at three heights: near the top, at the middle, and near the base. Each panel shows a slightly different reading on the display, with the three values written beneath. A note points at the widest reading and asks which one the part will actually be judged by]**
>
> *Figure 1.5.C - One measurement is an opinion. Three, taken at different places, tell you whether the part is even the same size all the way along.*
>
> *Alt text: Three panels showing a printed cylinder measured at top, middle and base, each giving a slightly different reading.*
...
> **[F5 force and motion overlay - Figure 1.5.D: a side view of a steering arm drawn in solid line at one end of its travel and in ghosted outline at the other. The region between the two positions is shaded and labelled swept volume. A screw head is drawn just inside that shaded region, with a collision marker on it, showing something placed where the arm needs to move]**
>
> *Figure 1.5.D - The part is not just the space it occupies - it is every space it passes through on the way.*
>
> *Alt text: A steering arm shown at both ends of its travel with the swept region between them shaded and an obstruction marked.*
...
Draw a larger box around the component, leaving room for everything in the table below.

This larger box is its **packaging envelope**.

| The envelope must include | Why |
|---|---|
| The component itself | It has to fit in the first place |
| Wires and their bend | A wire forced into a tight turn strains its joint |
| Clearance | Nothing should touch that is not meant to |
| Access | A hand or tool must reach it to fit and remove it |
| Cooling | Some parts need air moving around them |
...
1. A number and a unit. "Fifty" is not a measurement; "50 mm" is.
2. Because they are small enough to describe the features we care about without needing decimals for everyday sizes, and every drawing, printer and caliper in the project already works in them.
3. The diameter runs all the way across a circle through its centre. The radius runs from the centre to the edge, so it is half the diameter.
4. Inside diameter, outside diameter and width.
5. The distance from the centre of one feature to the centre of another - used for mounting holes, because it does not change if the holes are drilled slightly larger.
6. When the part is large, the required accuracy is low, or the shape is awkward to reach with calipers - a rough length of wire, or the overall size of a chassis plate.
7. The apparent shift in a reading when you look at a scale from an angle instead of square on. The mark appears to line up with a different value than it really does.
8. Because any small offset in the tool is added to every reading you take. Zeroing removes that offset, and it costs one second.
9. The large lower jaws.
10. The small upper jaws.
11. With the thin depth rod that extends from the end of the calipers as they open, placed into the hole with the body resting square on the surface.
12. Because it may not be the same size all along. Measuring in several places reveals taper, ovality or damage that a single reading would hide.
13. Because they compress. How hard you squeeze changes the reading, so a soft part has no single true size unless the measuring force is controlled.
14. Nominal size is the name we give the size - a "5 mm" shaft. Actual size is what it really measures, which is always slightly different.
15. Because of small real variation in the part, small differences in how the tool is placed and how hard it is squeezed, and the limits of the tool itself.
16. The smallest change a measuring tool can display.
17. Because resolution is only what the display can *show*. Accuracy is how close the reading is to the truth, and that depends on calibration, technique, temperature and the tool's real quality.
18. The amount of doubt attached to a measurement - the range within which the true value probably lies.
19. Because plastic expands when warm and shrinks as it cools. Measuring a part straight off the build plate gives a size the part will not keep.
20. What was measured, the value and unit, the tool used, how many readings were taken, the date, and anything unusual about the conditions.
21. A size or position that strongly affects fit, function, safety or assembly - one where being wrong stops the part doing its job.
22. The whole space a part passes through as it moves, not just the space it occupies when still.
23. Because the envelope must also include room for its wires and their bends, its connector and the fingers that fit it, the strap, and the clearance needed to lift it out.
24. Because drawings can be out of date, describe a different revision, or simply be wrong - and the part in your hand is the one you have to fit. Measure the real component.
25. The servo body's length, width and height, its mounting hole positions and centre-to-centre spacing, the output shaft position, the space the horn sweeps through, and the room the cable needs to leave and bend.
