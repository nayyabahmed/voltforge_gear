...
![Diagrams of tension, compression, bending, shear and torsion shown with arrows on simple shapes.](../assets/figures/1.4/1.4.A-five-ways-to-load-a-part.svg)

*Figure 1.4.A - Five ways to load a part, and every real load is a mixture of them.*

<!-- figure brief (F5 force and motion overlay), kept for redraws: signature five-panel diagram - the same simple bar loaded five ways, with labelled force arrows: pulled apart (tension), squeezed (compression), bowed with top squeezed and bottom stretched (bending), sliced by offset opposing arrows (shear), twisted at each end (torsion). One everyday example pictured under each panel: rope, chair leg, ruler, scissors, towel. -->
...
![A beam bowing under a central load, its top edge labelled squeezed, its bottom edge stretched, and a neutral centre line.](../assets/figures/1.4/1.4.B-beam-bending-child-friendly.svg)

*Figure 1.4.B - Bending is tension and compression at the same time - one edge stretches, the other squeezes, and the middle does neither.*

<!-- figure brief (F5 force and motion overlay), kept for redraws: side view of a beam bowing downward under a central load - top edge labelled "squeezed (compression)", bottom edge labelled "stretched (tension)", dashed centre line labelled "neutral - neither" -->
...
> **[F2 cross-section - Figure 1.4.C: card stack with offset arrows - top arrow pushing right, bottom arrow pushing left, cards part-slid like a fanned deck]**
>
> *Figure 1.4.C - Shear slides one part of the material across another, exactly like fanning a deck of cards.*
>
> *Alt text: A stack of cards sliding apart under opposing arrows, one pushing right along the top and one left along the bottom.*
...
> **[F7 comparison array - Figure 1.4.D: photo of a real broken RC part - a snapped suspension arm or shock tower - with the fracture surface visible and the crack starting point arrowed]**
>
> *Figure 1.4.D - The fracture surface tells you where the break started - find that point and you have found the design fault.*
>
> *Alt text: A snapped RC suspension arm photographed with its fracture surface visible and the crack starting point arrowed.*
...
> **[F3 right-versus-wrong array - Figure 1.4.E: two columns side by side - one straight under a small load, one bowed sideways under a larger load, arrows pressing down on both]**
>
> *Figure 1.4.E - A long thin column under compression does not squash shorter - it suddenly folds sideways.*
>
> *Alt text: Two columns under load, one straight under a small load and one bowed sideways under a larger one.*
...
> **[F5 force and motion overlay - Figure 1.4.F: end-on views of beam cross-sections - flat strip, U-channel, box section, I-beam - drawn from the same amount of material, labelled from floppiest to stiffest]**
>
> *Figure 1.4.F - The same amount of material, arranged four ways, gives four very different stiffnesses.*
>
> *Alt text: End-on views of a flat strip, U-channel, box section and I-beam made from the same material, ordered floppiest to stiffest.*
...
> **[F2 cross-section - Figure 1.4.G: a flat plate flexing under a push, next to the same plate with two ribs underneath holding it flat]**
>
> *Figure 1.4.G - A rib does not add much material - it moves the material away from the middle, where bending is resisted.*
>
> *Alt text: A flat plate flexing under a push beside the same plate held flat by two ribs underneath.*
...
> **[F2 cross-section - Figure 1.4.H: two brackets side by side - one with a sharp inside corner and tightly bunched force-flow lines at the corner, one with a rounded fillet and smooth, evenly spaced flow lines]**
>
> *Figure 1.4.H - A sharp inside corner crowds the force into a tiny area. A fillet gives it room to spread.*
>
> *Alt text: Stress lines crowding at a sharp internal corner beside the same corner with a fillet.*
...
> **[F4 sequence strip - Figure 1.4.I: a bearing on a shaft with two labelled arrows - one pressing down across the shaft (radial), one pushing along the shaft (axial)]**
>
> *Figure 1.4.I - Bearings are loaded two ways, and a bearing sized for one can fail under the other.*
>
> *Alt text: A bearing on a shaft with a radial arrow pressing across the shaft and an axial arrow pushing along it.*
...
> **[F1 annotated photograph - Figure 1.4.J: photo of a fractured printed part next to an annotated close-up of its fracture surface - crack origin, growth lines and final snap region labelled]**
>
> *Figure 1.4.J - The broken face is evidence: a clean flat break, a stretched one and a layer separation each mean something different.*
>
> *Alt text: Three broken part faces showing a brittle break, a ductile break and a separation along layer lines.*
...
1. A force or combination of forces acting on a part.
2. A pulling load that stretches a part along its length.
3. A squashing load that presses a part together along its length.
4. One side is stretched and the other is squeezed, with a neutral layer between them where neither happens.
5. A load where two parts of the material are pushed in opposite directions across each other, like scissors cutting.
6. A twisting load along the length of a part.
7. Tension: a suspension strap or a tie rod being pulled. Compression: a spring or a chassis post carrying weight. Bending: a suspension arm hitting a bump. Shear: a bolt holding two plates that try to slide apart. Torsion: a driveshaft twisting under motor torque.
8. The route a force follows through a part or assembly, from where it enters to where it finally leaves.
9. Because landing is a dynamic load. The buggy's weight arrives suddenly and has to be stopped in a short distance, so the peak force is many times its resting weight.
10. How concentrated a force is inside the material - the force divided by the area carrying it.
11. Because the same force is being carried by less material. Stress is force divided by area, so shrinking the area raises the stress even though the force has not changed.
12. Strength is how much load a part can take before it fails. Stiffness is how much it bends under load. A part can be strong but flexible, or stiff but brittle - they are separate properties.
13. Bending or stretching that reverses when the load is removed, so the part returns to its original shape.
14. Bending or stretching that does not reverse. The part stays deformed after the load is gone, and never returns to its designed shape.
15. The sudden sideways collapse of a long, thin part under compression. It does not squash gradually; it folds without warning.
16. Because folding moves material away from the middle, and bending is resisted mostly by material furthest from the centre. The same amount of material in a better shape is far stiffer.
17. A place where stress crowds into a small area - typically a sharp internal corner, a notch or a sudden change of section. Cracks usually start there.
18. Because a fillet turns a sharp corner into a curve, so the stress spreads over more material instead of crowding into a point.
19. Failure caused by many repeated loads, each far below what would break the part in one go. A crack starts small and grows a little with every cycle until the remaining material cannot hold.
20. Slow, permanent deformation under a steady load held for a long time - especially when warm. The part passes every immediate test and sags weeks later.
21. Because printed plastics soften as they warm. A part that is stiff at room temperature can bend, creep or lose its shape in a hot car or in direct sun, at temperatures well below melting.
22. Because a printed part is weaker between its layers than along them. If the load pulls the layers apart, the part fails at a layer line; if the layers run along the load, the force has to break through solid plastic.
23. Designing a part to withstand more than it is expected to meet - so that variation in material, printing, loading or use does not immediately cause failure.
24. Because it fails first, on purpose, protecting something more expensive or harder to replace. A cheap bumper mount that snaps in a crash is better than a cracked chassis.
25. The part and material, where it broke, which way the layers ran, what the load was, whether the break was clean or stretched, how long it had been in service, and a photograph before anything is touched.
