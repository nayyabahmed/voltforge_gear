...
> **[Signature visual, F5 force and motion overlay - Figure 1.9.F: the engineering design cycle drawn as a continuous loop of six stages - ask, imagine, plan, create, test, improve - with the arrow from improve running back into ask rather than stopping. A deliberate gap where a finish box would be is marked with a note reading there is no finish box. A second, fainter loop inside shows a smaller, faster iteration happening within one pass]**
>
> *Figure 1.9.F - The loop has no exit. Every improvement raises the next question, which is why engineers talk about versions rather than finished designs.*
>
> *Alt text: The engineering design cycle drawn as a continuous six-stage loop with no finish box and a smaller inner loop.*
...
> **[F3 right-versus-wrong array - Figure 1.9.B: two requirement cards side by side. Left, weak: 'the mount should be strong' with a callout asking how strong, measured how, and how anyone would know it passed. Right, testable: 'the mount shall hold the servo without visible movement when a 20 N side load is applied for 30 seconds', with the same three questions ticked off]**
>
> *Figure 1.9.B - A requirement you cannot test is a wish. The difference is a number and a way to measure it.*
>
> *Alt text: Two requirement cards comparing a vague statement with a testable one carrying a value and a pass condition.*
...
> **[F7 comparison array - Figure 1.9.C: a two-column board. Left column headed requirements, listing what the design must achieve. Right column headed constraints, listing the limits it must work inside: budget, printer build volume, tools owned, time available. Arrows show requirements pushing outward and constraints pushing inward, meeting in a shaded region in the middle labelled the space where the answer lives]**
>
> *Figure 1.9.C - Requirements say what to reach for. Constraints say how far you are allowed to go. The design lives where they overlap.*
>
> *Alt text: Requirements and constraints in two columns with arrows meeting in a shaded region between them.*
...
> **[F7 comparison array - Figure 1.9.D: a worked decision matrix on grid paper. Three concepts run down the side, four weighted criteria across the top, scores filled in each cell and a total column at the right. The winning row is circled, and a handwritten note beside it reads that the matrix organises the argument rather than settling it]**
>
> *Figure 1.9.D - A decision matrix does not make the decision - it makes your reasoning visible enough to argue with.*
>
> *Alt text: A decision matrix with three concepts scored against four weighted criteria and a total column.*
...
> **[F4 sequence strip - Figure 1.9.E: five panels of one bracket evolving through iterations: a pencil sketch, a cardboard mock-up, a first print that cracks at a corner, the same part with a fillet added, and the final version in service. Under each panel, one line naming what that version was built to find out and what it taught]**
>
> *Figure 1.9.E - Each version exists to answer one question. That is what makes a fifth version better rather than merely later.*
>
> *Alt text: Five panels showing a bracket evolving from sketch through cardboard, a cracked print, a filleted revision and the final part.*
...
> **[F1 annotated photograph - Figure 1.9.A: a filled-in concept card for the sliding battery tray - a quick thumbnail drawing of the tray sliding into its rails at the top, with the "how it works", "advantages", "risks" and "questions" notes written underneath]**
>
> *Figure 1.9.A - A concept card holds one idea, its strengths and its risks - small enough to throw away, complete enough to compare.*
>
> *Alt text: A filled-in concept card for a sliding battery tray, showing a thumbnail sketch with its strengths and risks noted.*
...
1. Because a finished design always suggests the next question. Testing produces evidence, evidence suggests improvements, and improvements need testing again. There is no point at which nothing more could be learned.
2. A short written description of the problem to be solved, in terms of need rather than solution.
3. A problem describes what is wrong or needed. A solution describes one particular way of fixing it. Writing a solution as the problem quietly rules out every other approach before you have considered any.
4. Because the design has to work for them, not for you. What they can reach, lift, understand and be trusted with decides what the design is allowed to be.
5. That it states something measurable with a pass condition - a number, a method and a way to tell whether it was met.
6. A statement of something the design must *do*.
7. A statement of *how well* it must do it - a speed, a load, a time, a distance.
8. A statement about how it must connect to something else - mounting holes, plug types, mating dimensions.
9. A limit that cannot be moved: the printer's build volume, the money available, the law of physics involved.
10. A limit you would prefer to respect but could trade away if the benefit were large enough.
11. Studying existing solutions to the same problem to learn from them before designing your own.
12. Breaking a large problem into smaller problems that can each be solved and tested separately.
13. Because the first idea is rarely the best, and having only one means you are choosing between it and nothing. Three real alternatives make the choice a comparison rather than a default.
14. Deliberately generating many different ideas without judging them yet.
15. Deliberately narrowing many ideas down to one, using requirements and constraints as the filter.
16. A table that scores each concept against weighted criteria so the options can be compared consistently.
17. Because both the weights and the scores are judgements made by people. The matrix organises the reasoning and makes it visible - it does not turn opinions into facts.
18. Something that might go wrong, with some chance of happening and some cost if it does.
19. Something you are treating as true without having checked it. Assumptions are not wrong to make, but they are dangerous to forget.
20. To answer a question about the design cheaply, before committing time and material to the real thing.
21. A rough build made only to show that an idea can work at all, without worrying about finish, size or durability.
22. Because a prototype that tries to answer everything answers nothing clearly. If it fails you will not know which of the several things it was testing was at fault.
23. The question being tested, the independent, dependent and control variables, the procedure, and a pass or fail condition written before the test runs.
24. Something deliberately kept the same across every test, so that it cannot be the reason the results differed.
25. The one thing you deliberately change between tests.
26. The thing you measure to see what effect your change had.
27. Qualitative data describes - it rattles, it feels loose, it looks scorched. Quantitative data measures - 3.2 mm, 45 seconds, 12 grams. Both are useful, and the first often tells you what to measure.
28. The underlying reason something failed, as opposed to the symptom you first noticed.
29. You ask why the failure happened, then ask why *that* happened, and repeat about five times until you reach a cause you can actually change rather than another symptom.
30. Because if you change several and the result improves, you cannot tell which change was responsible - or whether one helped while another made things worse.
31. One trip round the design cycle: a version built, tested and learned from, feeding the next version.
32. Because they record what changed and why. Without them, a later reader - often you - cannot tell whether a feature is deliberate or left over, and may remove the very thing that fixed an earlier failure.
33. The simplest version that can still answer the question you are asking, with everything not needed for that question left out.
34. When a project quietly grows beyond what was agreed - extra features, extra requirements - until it becomes too large to finish.
35. An agreed, written statement of what has to be true before a task counts as finished, decided before the work starts.
36. A structured check of a design by other people before committing to it, to find problems while they are still cheap to fix.
37. Being able to link every requirement to the evidence that shows it was met - and to show that nothing was quietly dropped.
38. Because it produces evidence. A test that passes only tells you it did not fail this time; a test that fails tells you exactly where the design's limit is and what to change.
39. Because early decisions are made with the least information. Keeping them reversible means new evidence can still be acted on, instead of being ignored because too much has been built on the old assumption.
40. Because CAD is slow to change compared with a sketch or a cardboard mock-up, and it invites you to make detailed decisions before the basic ones are settled. Answer the cheap questions first, then model.
