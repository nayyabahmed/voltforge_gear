...
> **[Signature visual, F7 comparison array - Figure 1.6.A: four dartboards in a 2x2 grid - top-left "accurate AND precise" (tight cluster on the bullseye), top-right "precise, not accurate" (tight cluster near the edge), bottom-left "accurate on average, not precise" (loose spread around the centre), bottom-right "neither" (darts scattered everywhere)]**
>
> *Figure 1.6.A - Accurate and precise are two different questions, and a set of readings can fail either one independently.*
>
> *Alt text: Four dartboards showing accurate and precise, precise but not accurate, accurate but not precise, and neither.*
...
> **[F3 right-versus-wrong array - Figure 1.6.E: two notebook entries recording the same measurement of a printed bracket. Left, false precision: the value written as 42.8571 mm, with a callout noting the ruler used could only resolve to a millimetre. Right, honest: 43 mm, with the tool and the number of readings noted beside it]**
>
> *Figure 1.6.E - Extra decimal places do not make a measurement more true - they only make it look more certain than it is.*
>
> *Alt text: Two notebook entries of one measurement, one written to four decimal places from a ruler and one written honestly.*
...
> **[F3 right-versus-wrong array - Figure 1.6.B: two dot-strips above a number line marked with the true value - "random error" dots scattered evenly around the true value, "systematic error" dots clustered tightly but shifted to one side of it]**
>
> *Figure 1.6.B - Random error scatters readings around the truth. Systematic error moves every reading the same way, so it stays hidden inside a tidy-looking set.*
>
> *Alt text: Two dot strips above a number line, one scattered evenly around the true value and one clustered tightly but offset to one side.*
...
> **[F7 comparison array - Figure 1.6.C: a closed digital caliper, jaws touching, its display reading 0.18 mm instead of 0.00 - an arrow noting "this 0.18 mm is added to every measurement until you re-zero"]**
>
> *Figure 1.6.C - A tool that does not read zero when closed adds that error to every measurement until somebody re-zeros it.*
>
> *Alt text: A closed digital caliper reading 0.18 mm instead of zero, annotated to show the error is added to every reading.*
...
> **[F4 sequence strip - Figure 1.6.F: the same shaft measured five times in a row by the same person with the same calipers, each reading written beneath its panel. The five values differ slightly in the second decimal place. Beneath, the spread between highest and lowest is bracketed and labelled as the range]**
>
> *Figure 1.6.F - One reading tells you almost nothing. Five tell you how much your own measuring varies.*
>
> *Alt text: Five successive measurements of one shaft with slightly different readings and the range bracketed beneath.*
...
> **[F5 force and motion overlay - Figure 1.6.D: a dot-strip number line around 10 mm - four dots clustered tightly near 10.01 mm and one lone dot far out at 11.26 mm, circled and labelled "outlier - investigate, do not just delete"]**
>
> *Figure 1.6.D - An outlier is the most interesting reading in the set - it is either a mistake worth finding or a fault worth knowing about.*
>
> *Alt text: A number line with four readings clustered near 10.01 mm and one lone reading at 11.26 mm circled as an outlier.*
...
2. How closely repeated measurements agree with each other.
3. Yes. A tool with a zero error can give the same wrong reading every time - tightly grouped, and all of them wrong.
4. Error that varies unpredictably from reading to reading, scattering results around the true value.
5. Error that shifts every reading the same way by roughly the same amount - a consistent bias rather than scatter.
6. Because random error is as likely to be high as low, so averaging several readings lets the scatter partly cancel out.
7. Because the bias is present in every reading, so averaging just gives you a very reliable average of the same wrong value.
8. A tool reading something other than zero when it should read zero - for example calipers showing 0.03 mm when fully closed.
9. Comparing a measuring tool against a trusted reference to find out whether it reads correctly, and correcting or noting the difference.
10. How closely the same person, using the same tool and method, agrees with themselves when repeating a measurement.
11. How closely different people, or different tools, agree when measuring the same thing.
12. Because they squeeze the calipers with different force, hold them at slightly different angles, read the scale from different positions, or measure at slightly different places on the part.
13. Writing more decimal places than the tool or method can justify, which makes a rough measurement look precise.
14. Because the number of decimals is a claim about how well you know the value. Writing 43.00 mm from a ruler claims a certainty the ruler cannot deliver.
15. A reading that sits well away from the others in a set.
16. Because it might be a mistake - a slip, a misread, a badly placed tool - or it might be real and tell you something important about the part. Deleting it without looking throws away the most interesting reading.
17. The difference between the highest and lowest reading.
18. Being able to follow a measurement back to the tool, method, date and person that produced it, so the result can be checked later.
19. Verification asks whether the part matches the drawing. Validation asks whether it actually works in real use. A part can pass one and fail the other.
20. Because a bearing seat is a fit: being a few hundredths out changes whether the bearing is loose, correct or crushed. A decorative curve has no such consequence.
21. When several small errors in separate parts or measurements add together into one much larger error in the finished assembly.
22. Because their errors can all point the same way. Three parts each 0.2 mm oversize stack into an assembly 0.6 mm out, even though every part passed inspection on its own.
23. Measuring the same thing a second way, or with a different tool, to see whether the two results agree.
24. That the measurement is accurate enough for the decision it will be used for - no better and no worse. It is defined by the job, not by the tool.
25. Because a number is only as trustworthy as the way it was produced. A confident-looking figure from a badly zeroed tool is worse than an honest rough one, because it invites you to rely on it.
