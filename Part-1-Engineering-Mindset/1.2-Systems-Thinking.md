...
> **[F7 comparison array - Figure 1.2.C: a single buggy drawn in the centre with arrows leaving it in two groups. On one side, useful outputs in green: movement, speed, a change of direction. On the other, unwanted outputs in orange: heat from the motor, noise, vibration, tyre wear. Each arrow is labelled, and a note reads that no machine produces only the outputs you asked for]**
>
> *Figure 1.2.C - Every machine makes things you did not ask for. Good design reduces them; nothing removes them.*
>
> *Alt text: A buggy with green arrows for useful outputs like movement and orange arrows for unwanted outputs like heat and wear.*
...
> **[F3 right-versus-wrong array - Figure 1.2.D: the same steering fault investigated with two different boundaries drawn as dashed rectangles. Left, too narrow: the boundary encloses only the servo, so the investigation stops there and the real cause - a loose linkage outside the box - is left outside and missed. Right, correctly drawn: the boundary encloses the servo, its linkage and the steering arms, and the loose joint is inside it]**
>
> *Figure 1.2.D - Draw the boundary too tightly and the cause of your problem ends up outside the box you are looking in.*
>
> *Alt text: Two dashed system boundaries around a steering fault, one excluding the loose linkage that caused it and one including it.*
...
> **[Signature visual, F1 annotated photograph - Figure 1.2.A: the real buggy photographed from above with its body off, and the six subsystems outlined in six different colours directly on the photograph: control, power, drive, steering, suspension and structure. Each outline is labelled, and thin leader lines connect each one to the matching box in a small copy of the system tree placed beside the photo]**
>
> *Figure 1.2.A - The tree and the machine are the same thing seen two ways - every abstract box on the left is a real group of parts on the right.*
>
> *Alt text: An overhead photograph of the buggy with six subsystems outlined in colour and linked to a system tree diagram beside it.*
...
> **[F5 force and motion overlay - Figure 1.2.E: a child riding a bicycle in a straight line, drawn from behind, with a loop of arrows overlaid: eyes see the bike drifting left, brain decides, hands steer slightly right, the bike moves, and the eyes see the result again. The loop is drawn as a continuous circle rather than a chain, and a caption marks the point where the correction happens]**
>
> *Figure 1.2.E - Feedback is a loop, not a line - the result of what you just did becomes the information for what you do next.*
>
> *Alt text: A cyclist seen from behind with a circular arrow loop showing eyes, brain, hands, bike movement and back to eyes.*
...
> **[F7 comparison array - Figure 1.2.F: a set of balance scales drawn three times, each weighing one pair of buggy qualities against another: strength against weight, speed against battery life, and grip against tyre wear. In each, adding to one pan visibly lifts the other. A note beneath reads that no design sits at the top of every pan at once]**
>
> *Figure 1.2.F - A trade-off is not a failure to decide - it is the honest shape of every real engineering choice.*
>
> *Alt text: Three sets of balance scales weighing strength against weight, speed against battery life, and grip against wear.*
...
> **[F2 cross-section - Figure 1.2.B: a close-up of a pinion gear meshing with a spur gear, with the teeth interlocking at the contact point magnified in an inset. The inset labels what actually crosses the boundary at that point - rotation and torque passing one way, and heat and wear appearing as unwanted outputs. A dashed line marks the interface itself]**
>
> *Figure 1.2.B - An interface is a place where two systems touch and something passes between them - here, tooth to tooth.*
>
> *Alt text: A pinion meshing with a spur gear with the contact point magnified, labelling the rotation passing across the interface.*
...
1. Because a machine is defined by how its parts are *connected*, not just by which parts exist. The same pile of correct parts, wired or bolted differently, does nothing at all. The connections are as much a part of the design as the components.
2. Input, process, output. Something goes in, something happens to it, something comes out.
3. Inputs: electrical energy from the battery, and commands from the driver's handset. Outputs: movement of the buggy, and heat from the motor. Bumps from the ground are also an input, and noise and tyre wear are also outputs.
4. An imaginary line you draw around the part of the world you are choosing to think about. Everything inside it is your system; everything outside it is the environment that supplies inputs and receives outputs.
5. Information about a result being fed back in to influence what happens next - like seeing your bicycle drift left and correcting the handlebars, then seeing the result of that correction.
6. Because a tight bearing adds friction. The motor has to work harder to turn against it, so it draws more current, and the extra electrical energy that is not becoming useful movement leaves as heat. The bearing and the motor are connected, so a problem in one shows up in the other.
7. A benefit gained by accepting a cost somewhere else. Making a part stronger usually makes it heavier; making it lighter usually makes it weaker. There is rarely a change that improves everything at once.
8. A requirement is something the design *must achieve* - what it has to do. A constraint is a limit you have to work within - budget, printer size, available tools, time. Requirements describe the goal; constraints describe the boundaries of the space you can search for it.
9. Any three, for example: where the pinion meets the spur gear, where the servo horn connects to the steering linkage, where the battery connector plugs into the speed controller, where the tyre meets the ground, or where a bolt joins a mount to the chassis.
10. Because a system map is cheap to draw and cheap to change. It exposes missing connections, unclear boundaries and dependencies before any time has been spent modelling geometry. Finding a design problem in a five-minute sketch costs five minutes; finding it in a finished CAD model costs a rebuild.
