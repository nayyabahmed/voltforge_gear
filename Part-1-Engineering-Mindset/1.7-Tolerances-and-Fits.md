...
> **[F5 force and motion overlay - Figure 1.7.A: a number line from 9.90 to 10.10 mm with 10.00 mm nominal marked in the middle and the whole band between 9.90 and 10.10 shaded as the "acceptable zone" - anything outside the shaded band is rejected]**
>
> *Figure 1.7.A - Every real dimension lives inside a band, and the whole band has to work.*
>
> *Alt text: A number line showing nominal size with upper and lower limits marked and the tolerance band shaded.*
...
> **[Signature visual, F2 cross-section - Figure 1.7.B: three shaft-in-hole cross-sections side by side. CLEARANCE - hole clearly bigger than the shaft, a gap all around, labelled "slides / spins". TRANSITION - hole and shaft almost the same size, barely a gap, labelled "light push". INTERFERENCE - shaft bigger than the hole, the material squeezed around it, labelled "pressed in, grips". This is the signature image of the topic.]**
>
> *Figure 1.7.B - The same shaft and hole, three fits - the difference is measured in hundredths of a millimetre.*
>
> *Alt text: Three cross-sections comparing clearance, transition and interference fits between a shaft and hole.*
...
> **[F2 cross-section - Figure 1.7.C: a chassis edge drawn as a datum line, with four holes A, B, C, D each dimensioned by its own arrow measured straight from that datum line - so no hole's position depends on any other hole (baseline dimensioning)]**
>
> *Figure 1.7.C - Measure every hole from the same edge and no hole's position depends on any other - so one error stays one error.*
>
> *Alt text: Four holes each dimensioned by its own arrow measured straight from a single chassis datum edge.*
...
> **[F6 measured drawing - Figure 1.7.D: the same four holes A, B, C, D dimensioned in a chain - A to B, B to C, C to D - so each hole's position is measured from the previous one and any error is passed along the chain to the last hole]**
>
> *Figure 1.7.D - Measuring everything from one datum keeps each error local instead of letting them add up.*
>
> *Alt text: A chassis outline dimensioned from a single datum beside the same outline dimensioned in a chain.*
...
> **[F2 cross-section - Figure 1.7.E: two hole entrances side by side - a sharp square-edged hole with a peg catching on the rim, and a chamfered hole whose sloped lead-in guides the same peg smoothly inside]**
>
> *Figure 1.7.E - A chamfer gives the part somewhere to start, turning a jam into a guided entry.*
>
> *Alt text: A cross-section of a chamfered hole entry guiding a pin in beside a square-edged hole catching it.*
...
> **[F6 measured drawing - Figure 1.7.F: a small printed rectangular block with a row of six labelled holes (5.00 to 5.50 mm) through it, and a single steel pin being tried in each hole in turn to find the best fit]**
>
> *Figure 1.7.F - A coupon tests the fit before the part exists - a row of holes in small steps, printed once.*
>
> *Alt text: A test coupon plate with a row of holes in small size increments, each labelled.*
...
1. Because every process varies. Tools wear, material moves, temperature changes and machines have limits, so no two parts are ever identical - only close enough.
2. The amount a dimension is allowed to vary and still be acceptable.
3. The named, target size of a feature - the number you design to, before any allowance for variation.
4. The largest and smallest sizes a dimension may be and still pass. Anything outside them is a reject.
5. Bilateral allows variation both above and below nominal, such as 10.0 ± 0.2. Unilateral allows it mainly in one direction, such as 10.0 +0.0 / -0.3.
6. A fit where the hole is larger than the shaft, so the parts assemble easily and can move.
7. A fit near the boundary between clearance and interference, where the parts may be slightly loose or slightly tight depending on where each lands in its tolerance band.
8. A fit where the shaft is larger than the hole, so the parts must be forced together and grip each other.
9. Clearance = hole size − shaft size.
10. Total clearance is the whole gap across the diameter. Per-side clearance is half of it, the gap on each side. Confusing the two is a common way to double an error.
11. Because the nozzle compresses each perimeter layer as it prints, squashing the extruded circle outward into the hole, so the finished opening comes out undersized - typically by 0.1 to 0.3 mm.
12. Because the right value depends on your printer, nozzle, material, layer height and settings. A number that works for someone else's machine is a starting point for a test, not an answer.
13. A fit loose enough for a part to rotate or slide freely in normal use.
14. A fit that positions a part accurately without needing it to move - close, but not forced.
15. A fit tight enough that the parts are pressed together and held by friction alone.
16. Because printed plastic is much weaker than metal and does not spring back the same way. A press fit can split the part outright, or slowly creep and become loose after a few weeks under load.
17. The intentional size difference designed between two mating parts, chosen to produce the fit you want.
18. Checking what happens when every dimension lands at its worst permitted value at the same time - the tightest possible fit and the loosest possible fit.
19. When the tolerances of several parts add together across an assembly, so the total variation is much larger than any single part's.
20. A reference surface, edge or line that other dimensions are measured from.
21. Baseline dimensioning measures every feature from one common datum, so each error stays local. Chain dimensioning measures each feature from the previous one, so errors accumulate along the chain.
22. Because grit gets into gaps. A fit designed with no room for dirt will bind outdoors, so moving parts may need more clearance than a clean bench test suggests.
23. The slight outward bulge at the bottom edge of a print, caused by the first layer being squashed onto the build plate.
24. Because they guide parts together at the start of assembly, turning a jam into a smooth entry, and because a chamfer on a bottom edge hides elephant's foot.
25. A small printed test piece carrying just the feature you are unsure about - a row of holes, a slot, a wall - printed before committing to the real part.
26. The feature and nominal size, the printer, material and settings used, the size actually achieved, and what fit resulted - so the next part starts from evidence.
27. Because a retained fit uses a screw, clip or shoulder to hold the bearing, so it can be removed and replaced without forcing. A heavy press fit often damages the printed part on the way out.
28. Because batteries vary between brands and even between production runs, and a wrapper adds thickness. The datasheet describes a family; the pack in your hand is the one that has to fit.
29. Whether the hole is actually undersized, whether the pin is oversized, whether elephant's foot is narrowing the entry, whether either part needs a chamfer, and whether the two were printed with the same settings.
30. Because fit is chosen for function. A bearing that must rotate needs clearance; a tight fit stops it turning. The best fit is the one that lets the part do its job, which is often not the tightest one available.
