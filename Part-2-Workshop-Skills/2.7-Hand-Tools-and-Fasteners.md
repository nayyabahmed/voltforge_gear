...
learning_objectives:
  ...
  - Choose washers, nuts, captive nuts or threaded inserts for printed parts.
...
- Choose between a through-bolt, captive nut, self-tapping screw and heat-set insert.
...
- a servo (the motor that steers the front wheels - Topic 3.5) must be replaced
...
> **[Signature visual, F2 cross-section - Figure 2.7.1: a long shallow ramp drawn flat, with a small block being pushed up it and the effort arrow marked short. Beside it, the same ramp wrapped around a cylinder to form a screw thread, with the matching turning arrow. A second panel cuts through a tightened joint to show the screw stretched very slightly like a stiff spring, pulling the two parts together, with the clamp force arrows drawn between the faces]**
>
> *Figure 2.7.1 - A screw is a ramp wrapped round a cylinder - and once tight, it works as a stretched spring holding the joint closed.*
>
> *Alt text: A flat ramp beside the same ramp wrapped into a screw thread, with a cross-section showing a tightened screw acting as a spring.*
...
> **🤔 Think about it.** Screw a jar lid on gently, then try it again
> as hard as you possibly can. Which one comes off more easily next time -
> and which one felt more secure while you were tightening it? Now imagine
> the lid is a printed plastic bracket rather than a metal ring.

Tightening harder feels like it is doing more. Past a point it is doing damage instead: crushing the plastic under the head, stretching the screw, or stripping the thread. A joint held by a crushed pad is looser tomorrow than one tightened correctly today.
...
> **[F2 cross-section - Figure 2.7.2: a cut-through of a screw joint with four numbered callouts: 1, the head bearing on the top part; 2, the shank passing through a clearance hole without touching its sides; 3, the thread engaged in the lower part or nut; 4, the clamp force squeezing the two faces together, drawn as opposing arrows in the joint line]**
>
> *Figure 2.7.2 - Four separate jobs in one small fastener - and a joint fails when any one of them is not done.*
>
> *Alt text: Cross-section of a screw joint with the head bearing, clearance hole, thread engagement and clamp force each numbered.*
...
> **[F7 comparison array - Figure 2.7.3: a row of common metric screws laid out at true scale: M2, M2.5, M3, M4 and M5, each labelled with its diameter and typical coarse pitch. Beneath, two magnified thread profiles side by side showing a correct pitch match seating fully, and a near-match where the crests and roots do not line up and the threads ride over each other]**
>
> *Figure 2.7.3 - Diameter alone does not identify a screw - a thread of the wrong pitch will start, then destroy both parts.*
>
> *Alt text: A scale row of M2 to M5 screws with pitches labelled, and magnified profiles comparing a correct thread match with a mismatch.*
...
> **[F6 measured drawing - Figure 2.7.4: side views of a socket-head screw and a countersunk screw at the same scale. Dimension arrows show that socket-head length is measured from under the head, while countersunk length is measured from the top of the head, because the head sinks into the part. A designation M3 x 12 is broken out with leader lines naming the M, the 3 mm diameter and the 12 mm length]**
>
> *Figure 2.7.4 - Where the length is measured from depends on the head - which is why the same number can mean two different screws.*
>
> *Alt text: Side views of socket-head and countersunk screws showing where length is measured from, with an M3 x 12 designation labelled.*
...
> **☕ Good place to pause.**
> You can now read any metric fastener and say what it is.
> The next run is about the tools that turn them, and why fit matters so much.
...
> **[F3 right-versus-wrong array - Figure 2.7.5: three magnified pairs looking straight down into a screw head. Pair 1: a correctly sized hex key seated fully, flats in contact, shaded to show the driving faces. Pair 2: an undersized key rattling in the socket with visible gaps and rounded corners beginning. Pair 3: a cross-head driver of the wrong pattern sitting high in the recess, contacting only at the tips]**
>
> *Figure 2.7.5 - The tool either fills the recess or destroys it - there is very little in between.*
>
> *Alt text: Three magnified views into a screw head comparing a correctly seated driver, an undersized rattling key and a wrong-pattern driver.*
...
> **[F1 annotated photograph - Figure 2.7.6: the starter hand-tool kit laid out flat on a bench in a single overhead shot, each item labelled: hex key set, small screwdrivers in the common driver profiles, side cutters, small pliers, a nut driver, and a parts tray. A dashed boundary separates the buy-now group from a smaller buy-when-needed group placed to one side]**
>
> *Figure 2.7.6 - This is the whole kit for a long time - each item earns its place by unlocking a task you cannot otherwise do.*
>
> *Alt text: Overhead photograph of a small hand-tool kit with each tool labelled and a buy-later group separated.*
...
> **[F4 sequence strip - Figure 2.7.7: four panels of a screw being started in a printed boss. Panel 1: the screw held upright and turned backwards, with a small arrow showing the anticlockwise motion. Panel 2: the moment the screw drops into the existing thread, marked with a click symbol. Panel 3: the screw turned forwards, running in straight. Panel 4, contrast: the same screw started forwards immediately and going in visibly tilted, with torn thread material shown at the hole mouth]**
>
> *Figure 2.7.7 - Turning backwards first lets the screw find the thread that already exists, instead of cutting a second one beside it.*
>
> *Alt text: Four panels showing a screw turned backwards until it drops into the thread, run in straight, and a tilted cross-threaded contrast.*
...
> **☕ Good place to pause.**
> That is the tool side covered.
> What follows is everything that goes around the screw to keep the joint honest.
...
> **[F3 right-versus-wrong array - Figure 2.7.8: two cross-sections of a screw clamping a printed plate. Left, wrong: a small screw head sinking into the plastic, the surface crushed and dimpled around it, layer lines visibly compressed. Right, correct: the same screw with a washer under the head, the load spread over a wide area and the plastic surface flat and undamaged. Contact area is shaded in both so the difference in size is obvious]**
>
> *Figure 2.7.8 - A washer is a snowshoe - the same force spread over more plastic stops the head sinking in.*
>
> *Alt text: Two cross-sections comparing a screw head crushing a printed plate against the same screw with a washer spreading the load.*
...
A **captive nut** sits inside a shaped pocket that stops it rotating or falling away.
...
> **☕ Good place to pause.**
> Metal-to-metal joints are straightforward.
> The next sections are about the harder problem: threads in printed plastic.
...
> **[F4 sequence strip - Figure 2.7.9: four panels of a heat-set insert being installed. Panel 1: the printed boss with its pilot hole, insert sitting on top. Panel 2: a temperature-controlled iron tip resting on the insert, the insert sinking as the plastic softens, with a hand shown keeping it vertical. Panel 3: the insert flush with the surface and the plastic re-solidified around its knurling. Panel 4: a screw threaded into the metal insert. An adult-supervision marker sits on panel 2]**
>
> *Figure 2.7.9 - The heat melts a path and the plastic closes around the knurling - which is why it goes in slowly and straight.*
>
> *Alt text: Four panels showing a heat-set insert placed, melted into a printed boss with an iron, seated flush, and receiving a screw.*
...
| Captive nut | One tool can tighten the joint | Pocket must fit and retain nut | Excellent after coupon test |
...
| Captive nut plus correct tightening | Printed modules | Removable |
...
In plastic threads they do not simply fail to work - many of them attack the plastic itself.

> **⚠️ SAFETY**
>
> **Do not put ordinary threadlocker anywhere near a printed part.**
> The common anaerobic threadlockers - including the familiar blue and red
> grades - chemically attack several plastics, especially ABS, polycarbonate
> and nylon. They cause stress cracking: the plastic turns brittle and can
> crumble, sometimes within minutes, and often at a joint you are relying on.
> A part can look perfect and then break in your hand.
>
> On printed parts, solve loosening mechanically instead: a nyloc nut, a
> captive nut, a washer that spreads the load, or a heat-set insert giving a
> proper metal thread. If a chemical really is needed, only a product the
> manufacturer states is plastic-safe may be used, chosen and applied by a
> responsible adult.
...
- an insert rotating
- a captive nut crushing its pocket
...
A servo (Topic 3.5) or drive motor (Topic 3.7) may require its own manufacturer-specified screw.
...
> **☕ Good place to pause.**
> The knowledge is done.
> The activities below put it into your hands - start with the door.
...
Use a pencil stub or rolled-paper tube as a pretend bolt and a square card block as a pretend captive nut.
...
> **[F3 right-versus-wrong array - Figure 2.7.10: ten compact rows of paired panels, one per mistake above, consistent viewing angle within each pair. Examples: a screw started crooked and cross-threading beside one started by turning backwards until it clicks; an undersized driver rounding a socket beside a correctly seated one; a screw tightened until the plastic crushes beside one tightened to firm; a self-tapping screw forced into an undersized hole beside a correctly sized pilot; ordinary threadlocker on a printed part beside a nyloc nut doing the same job mechanically]**
>
> *Figure 2.7.10 - Every pair is the same joint made twice - the right-hand version is still serviceable a year later.*
>
> *Alt text: Ten right-versus-wrong pairs covering thread starting, driver fit, over-tightening, pilot holes, washers, captive nuts and threadlocker on plastic.*

# Thinking Like an Engineer - The Joint Is the Product

A screw is not the point. The joint is.

When a fastener works loose on the buggy, the instinct is to reach for a bigger screw or more force. Both usually make it worse: a bigger screw needs a bigger hole in a part that is already thin, and more force crushes the plastic that was holding everything together.

Engineers ask a different question. What is the joint actually failing to do - is it losing clamp force, is the thread stripping, or is the part around it bending? Each of those has a different answer. Losing clamp force wants a nyloc nut or a washer. A stripping thread wants a heat-set insert. A bending part wants more material, not more screw.

> Fix the joint, not the fastener.

Write the failure in your notebook before you change anything: which screw, what came loose, what it was holding, and what you changed. Three entries in and you will start seeing the same cause repeatedly - and that pattern is worth more than any single fix.

---

# Stop Point Before Buying More Tools

You now have enough to assemble almost every joint on the buggy.

Before spending anything else, check that:

- every screw you own has a driver that fits it properly
- your fasteners are sorted and labelled by size, not by rough appearance
- you have washers in the sizes you actually use
- you have recorded which screw goes where on the buggy

A tidy, complete small kit beats a large incomplete one. The next real purchase is driven by Topic 2.8, when cutting and drilling begin - not by a tempting set with sixty pieces you will never open.
...
- Washers spread force; nyloc nuts resist rotation; captive nuts improve access; heat-set inserts provide reusable metal threads in printed plastic.
...
| Captive nut | A nut held in a shaped pocket so it cannot rotate or fall away. |
...
- [ ] I can compare a nut, captive nut, heat-set insert and self-tapping screw.
...
1. To create and hold clamp force - to squeeze the parts together hard enough that they behave as one piece. The screw is not a peg holding parts in place; it is a spring holding them shut.
2. Because the thread is a ramp wrapped around a cylinder. A long shallow ramp lets a small push raise a heavy load slowly, and a screw does the same: a small turning effort at the head becomes a large pull along the screw.
3. A metric screw with a 3 mm nominal thread diameter and a 12 mm length. It does not tell you the head shape, the driver profile or the thread pitch, all of which you still have to match.
4. A socket-head screw is measured from under the head, because the head sits on the surface. A countersunk screw is measured from the top of the head, because the head sinks into the part and becomes flush.
5. The long arm gives extra leverage, which helps to loosen a tight screw. That same leverage makes it very easy to apply far more torque than a small screw or a printed part can survive, so it is the wrong end to use for tightening.
6. Stop and find the correct driver. The wobble means the tool is only touching at a few points, so the force is concentrated there and will round out the recess - after which no driver will grip it.
7. Any three of: the screw suddenly becomes hard to turn after starting easily, it goes in visibly tilted, you can see torn or curled material at the hole mouth, the screw will not seat fully, or it turns with a gritty rather than a smooth feel.
8. Because PETG is softer than the steel screw head. Without a washer the head sinks into the plastic and dimples it, so the clamp force falls away as the plastic creeps. A washer spreads the same force over a much larger area.
9. The screw has bottomed out - it has run out of hole before the head reached the surface, so it is pushing against the bottom rather than pulling the parts together. Use a shorter screw, or make the hole deeper if the part allows it.
10. A through-bolt and nut is cheap, strong and needs no special tools, but you need access to both sides and the nut can be dropped or lost. A heat-set insert gives a proper metal thread accessible from one side and survives repeated removal, but it needs an iron, adult supervision and a correctly sized boss. For a cover removed often from one side, the insert is usually worth it.
11. Because a self-tapping screw cuts its own thread in the plastic every time it goes in. Each removal and refit damages that thread a little more, so after a few cycles it no longer holds. A heat-set insert or a captive nut is the right answer for anything removed regularly.
12. Any four of: is the screw the correct length and not bottoming out; is there enough clamp force, or is the plastic under the head being crushed; is the thread stripped in the plastic; is there vibration that needs a nyloc nut or a captive nut; is the part itself flexing rather than the joint loosening; and is the screw the one the motor manufacturer specified.
